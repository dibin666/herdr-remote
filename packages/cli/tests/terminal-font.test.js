...
function fakeRun(table) {
  return (command, args) => {
    ...
    return Object.hasOwn(table, key) ? { status: 0, stdout: table[key] } : { status: 1, stdout: '' };
  };
}
...
test('Pango descriptions give up their family and size in pixels', () => {
  assert.deepEqual(parsePangoFontDescription('JetBrainsMono Nerd Font 9'), { family: 'JetBrainsMono Nerd Font', sizePx: 12 });
  assert.deepEqual(parsePangoFontDescription('Source Code Pro Semi-Bold Italic 10.5'), { family: 'Source Code Pro', sizePx: 14 });
  assert.deepEqual(parsePangoFontDescription('Hack 16px'), { family: 'Hack', sizePx: 16 });
  assert.deepEqual(parsePangoFontDescription('Fira Code, Noto Sans Mono 11 @wght=450'), { family: 'Fira Code', sizePx: 14.7 });
  assert.deepEqual(parsePangoFontDescription('Monospace'), { family: 'Monospace', sizePx: undefined });
  assert.equal(parsePangoFontDescription(''), null);
});

test('Qt and fontconfig font strings are read the same way', () => {
  assert.deepEqual(parseQtFontString('JetBrains Mono,10,-1,5,400,0,0,0,0,0'), { family: 'JetBrains Mono', sizePx: 13.3 });
  assert.deepEqual(parseQtFontString('Hack,-1,15,5,50,0,0,0,0,0'), { family: 'Hack', sizePx: 15 });
  assert.deepEqual(parseFontconfigPattern('JetBrains Mono:size=10:weight=bold,Noto Color Emoji:size=10'), { family: 'JetBrains Mono', sizePx: 13.3 });
  assert.deepEqual(parseFontconfigPattern('monospace:pixelsize=14'), { family: 'monospace', sizePx: 14 });
});
...
test('the nearest terminal process wins over variables leaked from an outer one', () => {
  ...
  assert.equal(identifyTerminal({ env: { GNOME_TERMINAL_SCREEN: 'x' }, ancestry: ['herdr', 'systemd'] }), 'gnome-terminal');
  assert.equal(identifyTerminal({ env: { TERM_PROGRAM: 'vscode', GNOME_TERMINAL_SCREEN: 'x' }, ancestry: [] }), 'vscode');
  assert.equal(identifyTerminal({ env: { TERM: 'foot' }, ancestry: [] }), 'foot');
  assert.equal(identifyTerminal({ env: { TERM: 'xterm-256color' }, ancestry: ['bash', 'sshd'] }), null);
});
...
test('GNOME Terminal: the default profile font, or the desktop font when it says so', () => {
  ...
  assert.deepEqual(readTerminalFont('gnome-terminal', { ...base, run: fakeRun(table) }), {
    source: 'gnome-terminal', family: 'JetBrainsMono Nerd Font', sizePx: 12,
  });

  table[`gsettings get ${profile} use-system-font`] = 'true\n';
  assert.deepEqual(readTerminalFont('gnome-terminal', { ...base, run: fakeRun(table) }), {
    source: 'gnome-terminal', family: 'Noto Sans Mono', sizePx: 13.3,
  });
...
test('config-file terminals: kitty, Alacritty, Ghostty, foot, Konsole, VS Code', () => {
  ...
  const files = {
    ...
    [`${config}/ghostty/config`]: 'font-family = Iosevka\nfont-family =\nfont-family = "Maple Mono"\nfont-size = 15\n',
    ...
    [`${config}/Code/User/settings.json`]: '{ "terminal.integrated.fontFamily": "\'MesloLGS NF\', monospace", "editor.fontSize": 13, }',
  };
  const deps = { ...base, readFile: fakeFiles(files), run: fakeRun({}) };

  assert.deepEqual(readTerminalFont('kitty', deps), { source: 'kitty', family: 'Cascadia Code', sizePx: 13.3 });
  assert.deepEqual(readTerminalFont('alacritty', deps), { source: 'alacritty', family: 'Hack', sizePx: 16 });
  assert.deepEqual(readTerminalFont('ghostty', deps), { source: 'ghostty', family: 'Maple Mono', sizePx: 20 });
  assert.deepEqual(readTerminalFont('foot', deps), { source: 'foot', family: 'Fira Code', sizePx: 12 });
  assert.deepEqual(readTerminalFont('konsole', deps), { source: 'konsole', family: 'Hack', sizePx: 14.7 });
  assert.deepEqual(readTerminalFont('vscode', deps), { source: 'vscode', family: 'MesloLGS NF', sizePx: 13 });
});

test('a terminal left on its defaults reports the default it really draws with', () => {
  const deps = { ...base, readFile: fakeFiles({}), run: fakeRun({ 'fc-match -f %{family[0]} monospace': 'DejaVu Sans Mono' }) };
  // Ghostty embeds JetBrains Mono; kitty and foot ask fontconfig for "monospace".
  assert.deepEqual(readTerminalFont('ghostty', deps), { source: 'ghostty', family: 'JetBrains Mono', sizePx: 16 });
  assert.deepEqual(readTerminalFont('kitty', deps), { source: 'kitty', family: 'DejaVu Sans Mono', sizePx: 14.7 });
  assert.deepEqual(readTerminalFont('foot', deps), { source: 'foot', family: 'DejaVu Sans Mono', sizePx: 10.7 });
});
...
test('font files: real TTF/OTF of the right family only, never the same file twice', (t) => {
  ...
  const run = fakeRun({
    'fc-match -f %{file}\n%{family}\n%{index}\n My Mono:weight=regular:slant=roman': answer(regular, 'My Mono,My Mono NF'),
    'fc-match -f %{file}\n%{family}\n%{index}\n My Mono:weight=bold:slant=roman': answer(bold, 'My Mono'),
    // No italic: fontconfig falls back to the regular file.
    'fc-match -f %{file}\n%{family}\n%{index}\n My Mono:weight=regular:slant=italic': answer(regular, 'My Mono'),
    // No bold italic either: fontconfig picks some other family entirely.
    'fc-match -f %{file}\n%{family}\n%{index}\n My Mono:weight=bold:slant=italic': answer(other, 'DejaVu Sans Mono'),
  });

  const faces = resolveFontFaces('My Mono', { ...base, run });
  assert.deepEqual(faces.map(({ style, format, bytes }) => ({ style, format, bytes })), [
  ...
  const ttc = fakeRun({ 'fc-match -f %{file}\n%{family}\n%{index}\n CJK:weight=regular:slant=roman': answer(regular, 'CJK', 2) });
  assert.deepEqual(resolveFontFaces('CJK', { ...base, run: ttc }), []);
  const woff = fontFile(directory, 'Web.woff2', 'wOF2');
  const web = fakeRun({ 'fc-match -f %{file}\n%{family}\n%{index}\n Web:weight=regular:slant=roman': answer(woff, 'Web') });
...
test('the font detected at the entry point is inherited and remembered', (t) => {
  ...
  let asked = false;
  captureTerminalFont({ env, detect: () => { asked = true; return null; } });
...
test('the connector serves slices and re-reads the font for a window that asks', (t) => {
  ...
  const record = { family: 'Tiny Mono', sizePx: 12, source: 'kitty', faces: resolveFontFaces('Tiny Mono', { ...base, run }) };
  ...
  const connector = new HostConnector({
    ...
    config: { herdr: { args: [], cwd: process.cwd(), socketPath: null }, cleanup: { heartbeatIntervalMs: 10 } },
  });
  const sent = [];
  connector.ws = { readyState: WebSocket.OPEN, send(payload) { sent.push(JSON.parse(payload)); }, close() {} };
  const deliver = (message) => connector.handleMessage(Buffer.from(JSON.stringify(message)), false);

  deliver({ type: 'host_font_chunk_request', clientId: 'stream-1', sha256: record.faces[0].sha256, index: 0 });
  ...
  deliver({ type: 'host_font_chunk_request', clientId: 'stream-1', sha256: record.faces[0].sha256, index: 1 });
...
test('Terminal.app: the default profile’s archived NSFont', () => {
  ...
  const run = fakeRun({ 'fc-list :postscriptname=JetBrainsMonoNF\\-Regular family': 'JetBrainsMono Nerd Font,JetBrainsMono NF\n' });
  const deps = { ...base, platform: 'darwin', run, readBuffer: (file) => (file === prefs ? fixture : null) };

  assert.equal(identifyTerminal({ env: { TERM_PROGRAM: 'Apple_Terminal' }, ancestry: [] }), 'apple-terminal');
  assert.deepEqual(readTerminalFont('apple-terminal', deps), {
    source: 'apple-terminal', family: 'JetBrainsMono Nerd Font', sizePx: 17.3,
  });
...
test('iTerm2: the default profile’s font, by its PostScript name', () => {
  ...
  const run = fakeRun({ 'fc-list :postscriptname=JetBrainsMonoNF\\-Regular family': 'JetBrainsMono Nerd Font\n' });
  const deps = { ...base, platform: 'darwin', run, readBuffer: (file) => (file === prefs ? fixture : null) };
  assert.deepEqual(readTerminalFont('iterm2', deps), { source: 'iterm2', family: 'JetBrainsMono Nerd Font', sizePx: 18 });
});
...
test('Apple’s own faces are named without a font scan and never offered as files', (t) => {
  ...
  const fixtureless = readTerminalFont('apple-terminal', { ...deps, readBuffer: () => parseableEmptyPlist() });
...
test('xterm and urxvt: the emulator’s command line over the X resources', () => {
  const xrdb = fakeRun({
    'xrdb -query': 'XTerm*faceName:\tDejaVu Sans Mono\nXTerm*faceSize:\t10\nURxvt.font:\txft:Iosevka Term:size=12,xft:Noto Color Emoji\n*faceName:\tIgnored\n',
  });
  const deps = { ...base, run: xrdb };
  assert.deepEqual(readTerminalFont('xterm', deps), { source: 'xterm', family: 'DejaVu Sans Mono', sizePx: 13.3 });
  assert.deepEqual(
    readTerminalFont('xterm', { ...deps, terminalArgs: ['xterm', '-fa', 'JetBrains Mono', '-fs', '11'] }),
  ...
  assert.deepEqual(readTerminalFont('urxvt', deps), { source: 'urxvt', family: 'Iosevka Term', sizePx: 16 });

  // Bitmap core fonts cannot be drawn by a browser: no answer, not a guess.
  const bitmap = { ...base, run: fakeRun({ 'xrdb -query': 'URxvt.font:\t-misc-fixed-medium-r-normal--13-120-75-75-c-70-iso10646-1\n' }) };
...
test('large fonts are offered for cutting: the CJK fallback, or a family too big to send', (t) => {
  ...
  const latin = fakeRun({
    ...
    [query('Tiny Mono:charset=4e00:weight=regular:slant=roman')]: answer(cjk, 'Noto Sans Mono CJK SC,Noto Sans Mono CJK SC Regular', 7),
  });
  ...
  const sarasa = fakeRun({
    ...
    [query('Sarasa Mono SC:charset=4e00:weight=regular:slant=roman')]: answer(cjk, 'Sarasa Mono SC', 3),
  });
  assert.deepEqual(resolveSubsetSources('Sarasa Mono SC', [], { ...base, run: sarasa }).map((item) => item.scope), ['all']);
...
test('the connector answers a cut inline when small, and in slices when large', (t) => {
  ...
  const source = { family: 'Noto Sans CJK SC', style: 'regular', scope: 'cjk', sha256: 'c'.repeat(64), path: file, index: 0, bytes: stat.size, mtimeMs: stat.mtimeMs };
  const asked = [];
  const connector = new HostConnector({
    relayUrl: 'ws://127.0.0.1:1/ws/host',
    hostId: 'host-test',
  ...
  deliver({ type: 'host_font_subset_request', clientId: 's1', requestId: 'r1', sha256: source.sha256, text: '你好你' });
  ...
  deliver({ type: 'host_font_subset_request', clientId: 's1', requestId: 'r2', sha256: source.sha256, text: '一二三四' });
  ...
  deliver({ type: 'host_font_subset_request', clientId: 's1', requestId: 'r3', sha256: source.sha256, text: '你' });
...
test('HarfBuzz cuts a real font down to the requested characters', (t) => {
  ...
  const located = require('node:child_process').spawnSync('fc-match', ['-f', '%{file}', 'DejaVu Sans Mono'], { encoding: 'utf8' });
  ...
  const cut = subsetter.subset({ path: file, index: 0 }, [...'Herdr'].map((char) => char.codePointAt(0)));
  subsetter.release();
  assert.ok(cut.length > 100 && cut.length < fs.statSync(file).size / 10, `cut is ${cut.length} bytes`);
