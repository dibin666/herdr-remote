...
describe('PredictionLayer', () => {
  it("draws a program's own caret where the next key lands, in the program's style", () => {
    ...
    const layer = new PredictionLayer({ getTerminal: () => screen.terminal, isCursorHidden: () => true });
  ...
  it("covers the program's caret a round trip behind with plain text", () => {
    ...
    expect(cells).toContainEqual({ row: 5, col: 8, chars: ' ', width: 1, style: { fg: 0, bg: 0, ext: 0 } });
    expect(cells).toContainEqual({ row: 5, col: 7, chars: ' ', width: 1, style: { fg: INVERSE, bg: 0, ext: 0 } });
  });

  it('moves the terminal cursor itself when the program shows it', () => {
    const screen = fakeTerminal({ row: 2, col: 10 });
    const layer = new PredictionLayer({ getTerminal: () => screen.terminal, isCursorHidden: () => false });
    layer.sync([
  ...
  it('draws no caret when the program shows none', () => {
    ...
    expect(layer.paint()).toEqual({ cells: [{ row: 2, col: 10, chars: 'l', width: 1, style: typed }] });
  });
